...
parser.add_argument('--data_file', default='data/raw/vampire_jjt.pkl',
                    help='Data file used to compute splits')
...
def main():
    ...
    for prob in list(data.keys()):
        if len(data[prob]['axioms']) < args.min_axioms or len(data[prob]['axioms']) > args.max_axioms:
            del data[prob]
    print(f'{len(data)} problems remaining after filtering on axiom lengths min:{args.min_axioms} max:{args.max_axioms}')
